...
public class AndroidMnistDataFetcher extends BaseDataFetcher {
    ...
    protected static final String TEMP_ROOT = MainActivity.ROOT;
     ...
     * @throws IOException
     */
    public AndroidMnistDataFetcher(boolean binarize) throws IOException {
        this(binarize, true, true, System.currentTimeMillis());
    }

    public AndroidMnistDataFetcher(boolean binarize, boolean train, boolean shuffle, long rngSeed) throws IOException {
        if (!mnistExists()) {
            new AndroidMnistFetcher().downloadAndUntar();
        }
        String images;
        String labels;
        if (train) {
            images = MNIST_ROOT + AndroidMnistFetcher.trainingFilesFilename_unzipped;
            labels = MNIST_ROOT + AndroidMnistFetcher.trainingFileLabelsFilename_unzipped;
            totalExamples = NUM_EXAMPLES;
        } else {
            images = MNIST_ROOT + AndroidMnistFetcher.testFilesFilename_unzipped;
            labels = MNIST_ROOT + AndroidMnistFetcher.testFileLabelsFilename_unzipped;
    ...
    private boolean mnistExists() {
        //Check 4 files:
        File f = new File(MNIST_ROOT, AndroidMnistFetcher.trainingFilesFilename_unzipped);
        if (!f.exists())
            return false;
        f = new File(MNIST_ROOT, AndroidMnistFetcher.trainingFileLabelsFilename_unzipped);
        if (!f.exists())
            return false;
        f = new File(MNIST_ROOT, AndroidMnistFetcher.testFilesFilename_unzipped);
        if (!f.exists())
            return false;
        f = new File(MNIST_ROOT, AndroidMnistFetcher.testFileLabelsFilename_unzipped);
        if (!f.exists())
            return false;
        return true;
    }

    public AndroidMnistDataFetcher() throws IOException {
        this(true);
    }
